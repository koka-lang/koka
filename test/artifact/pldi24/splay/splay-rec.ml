...
let rec accessz t k =
  match t with
  | Node (l,x,r) ->
      if (x < k) then match r with
        | Node(rl,rx,rr) ->
            if rx < k then match accessz rr k with
                             | Root(rrl,rrx,rrr) -> Root(Node(Node(l,x,rl),rx,rrl),rrx,rrr)
            else if rx > k then match accessz rl k with
        ...
        | Node(ll,lx,lr) ->
            if lx < k then match accessz lr k with
                             | Root(lrl,lrx,lrr) -> Root(Node(ll,lx,lrl),lrx,Node(lrr,x,r))
            else if lx > k then match accessz ll k with
