...
while : ; do
  ...
  case "$flag" in
    ...
    -h|--help|-\?|help|\?)
        ...
        echo "  -n=N                         run each benchmark N times ($max_runs)"
        echo ""
        echo "mode:"
        echo "  build                        build the specified benchmarks"
        echo "  run                          run the specified benchmarks"
        echo ""
        echo "benchmarks:"
        echo "  allb                         all benchmarks (mtr,splay,zip,rbtree)"
        echo "  mtr                          move-to-root benchmarks"
        echo "  splay                        splay tree benchmarks"
        echo "  zip                          zip tree benchmarks"
        echo "  rbtree                       red-black tree benchmarks"
        echo ""
        echo "examples:"
        echo "  ./bench.sh build allb        build all benchmarks"
        echo "  ./bench.sh run -n=10 allb    run all benchmarks 10 times"
        echo "  ./bench.sh run -n=5 zip      run the zip benchmark 5 times"
        echo ""
        echo "(see the 'bench.sh' source for all available options)"
        echo ""
        exit 0;;
