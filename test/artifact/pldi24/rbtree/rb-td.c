...
// Daan: for best performance we modified the original sources to 
// - be intrusive with a direct key and value in the tree (to avoid extra element allocations)
// - have a static comparison function, no element free function, and a specialized fold such that all function calls are now static.
//
// This uses top-down red-black insertion.
...
typedef intptr_t node_key_t;
typedef intptr_t node_val_t;

static int key_compare( node_key_t k1, node_key_t k2 ) {
  int d = (int)k1 - (int)k2;
...
typedef int (*compare_fun_t)(node_key_t, node_key_t);
...
void *
__tsearch (node_key_t key, node_val_t val, node *vrootp)
{
  node q, root;
  ...
  if (q != NULL)
    {
	 ...
	 not obeying the malloc contract.  */
      ...
      SETRIGHT(q,NULL);
    #if ADDPARENT
      q->parent = (parentp == NULL ? NULL : *parentp);
    #endif
      if (nextp != rootp)
...
/* Find datum in search tree.
   ...
   COMPAR the ordering function.  */
void *
__tfind (node_key_t  key, void *const *vrootp)
{
  node root;
  node *rootp = (node *) vrootp;
...
void *
__tdelete (node_key_t key, void **vrootp)
{
  node p, q, r, retval;
  int cmp;
  node *rootp = (node *) vrootp;
  ...
  root = DEREFNODEPTR(rootp);
  while ((cmp = key_compare(key, root->key)) != 0)
    {
      if (sp == stacksize)
	{
	  node **newstack;
...
static void
tdestroy_recurse (node root)
{
  if (LEFT(root) != NULL)
    tdestroy_recurse (LEFT(root));
  if (RIGHT(root) != NULL)
    tdestroy_recurse (RIGHT(root));
  free (root);
}

void
__tdestroy (void *vroot)
{
...
const_node mk_map(unsigned n) {
  node t = NULL;
  while (n > 0) {
    --n;
    __tsearch( n, (n%10 == 0), (void**)&t );    
  }
  return t;
}

static unsigned  fold(const_node root)
{
  if (root == NULL) return 0;
   else return fold(LEFT(root)) + fold(RIGHT(root)) + (root->val ? 1 : 0);
}
...
static long long sum(const_node root)
{
  if (root == NULL) return 0;
   else return sum(LEFT(root)) + sum(RIGHT(root)) + root->val;
}
...
static node access(node t, node_key_t k ) {
  node root = t;
  __tsearch( k, k, &root );
  return root;
}
...
static long max_height(const_node t ) {
  if (t==NULL) return 0;
  return 1 + max(max_height(LEFT(t)), max_height(RIGHT(t)));    
}

static long min_height(const_node t ) {
  if (t== NULL) return 0;
  return 1 + min(min_height(LEFT(t)), min_height(RIGHT(t)));   
}

static long top(const_node t) {
  if (t==NULL) return 0;
            else return t->key;
}
