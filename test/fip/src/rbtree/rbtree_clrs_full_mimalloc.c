...
// When the tree is fully rebalanced, we continue to go up to the root along the parent pointers.
...
void insert(RedBlackTree *T, int32_t key, enum Bool value) {
    Node *z = (Node *)mi_malloc(sizeof(Node));
    z->key = key;
    z->value = value;
    Node *y = T->nil;
    Node *x = T->root;
...
RedBlackTree *empty_rbtree() {
    Node *nil = (Node *)mi_malloc(sizeof(Node));
    nil->color = BLACK;
    RedBlackTree *t = (RedBlackTree *)mi_malloc(sizeof(RedBlackTree));
    t->root = nil;
    t->nil = nil;
    return t;
}
