...
import Compile.Options       ( Flags(..), CC(..), BuildType(..), buildType, ccFlagsBuildFromFlags, unquote,
...
compileModule term flags modules name compileTarget importPath -- todo: take force into account
  = runIOErr $
    do let imp = ImpProgram (Import name name rangeNull rangeNull rangeNull Private False)
                             ...
                             Just (_,effName)
                               -> let defaultHandlerName = makeHiddenName "default" (unqualify effName)
                                  in -- trace ("looking up: " ++ show defaultHandlerName) $
                                     case gammaLookup defaultHandlerName (loadedGamma loaded) of
                                        [(_,fun@InfoFun{})]
