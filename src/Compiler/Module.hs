...
import Core.Borrowed          ( Borrowed, borrowedNew, borrowedEmpty, borrowedExtends, extractBorrowDefs )
    ...
    let core = modCore mod
        ...
        loaded
          = Loaded (gammaUnion gamma1 (extractGamma isValue False core))
                ...
                inlines1
                (borrowedExtends (extractBorrowDefs (Core.coreProgDefs core)) borrowed1)
    in (loaded,errs)
