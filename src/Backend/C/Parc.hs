...
dupDropFun False {-drop-} tp (Just (conRepr,_)) (Just scanFields) arg
  ...
  = App (Var (TName name coerceTp) (InfoExternal [(targetPlatformFromTarget (C CDefault), "dropn(#1,#2)")])) [arg,makeInt32 (toInteger scanFields)]
  where
    name = nameDrop
    coerceTp = TFun [(nameNil,tp),(nameNil,typeInt32)] typeTotal typeUnit
dupDropFun isDup tp mbConRepr mbScanCount arg
  = App (Var (TName name coerceTp) (InfoExternal [(targetPlatformFromTarget (C CDefault), (if isDup then "dup" else "drop") ++ "(#1)")])) [arg]
  where
