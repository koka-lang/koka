...
genDup :: TName -> Expr
genDup name
  = App (Var (TName nameDup coerceTp) (InfoExternal "dup(#1)")) [Var name InfoNone]
  where
    tp = typeOf name
...
genDropReuse tname scan
  = App (Var (TName nameDropReuse funTp) (InfoExternal "drop_reuse(#1,#2,kk_context())"))
...
genIsUnique tname
  = App (Var (TName nameIsUnique funTp) (InfoExternal "is_unique(#1)"))
        [Var tname InfoNone]
...
genFree :: TName -> Expr
genFree tname
  = App (Var (TName nameFree funTp) (InfoExternal "kk_constructor_free(#1,kk_context())"))
        [Var tname InfoNone]
  where funTp = TFun [(nameNil, typeOf tname)] typeTotal typeUnit
...
genReuseDrop tname
  = App (Var (TName nameReuseDrop funTp) (InfoExternal "kk_reuse_drop(#1,kk_context())"))
        [Var tname InfoNone]
...
genReuseNull :: Expr
genReuseNull
  = App (Var (TName nameReuseNull funTp) (InfoExternal "kk_reuse_null")) []
  where funTp = TFun [] typeTotal typeReuse

-- Generate a reuse a block
genReuseAddress :: TName -> Expr
genReuseAddress tname
  = App (Var (TName nameReuse funTp) (InfoExternal "reuse_datatype(#1,kk_context())")) [Var tname InfoNone]
  where
    tp    = typeOf tname
...
genReuseAssignWith reuseName arg
  = let assign = TName nameAssignReuse (TFun [(nameNil,typeReuse),(nameNil,typeReuse)] typeTotal typeUnit)
    in App (Var assign (InfoExternal "#1 = #2")) [Var reuseName InfoNone, arg]
