...
genDecRef tname
  = App (Var (TName nameDecRef funTp) (InfoExternal [(targetPlatformFromTarget (C CDefault), "decref(#1,current_context())")]))
        [Var tname InfoNone]
  where
...
genDup :: TName -> Expr
genDup name
  = App (Var (TName nameDup coerceTp) (InfoExternal [(targetPlatformFromTarget (C CDefault), "dup(#1)")])) [Var name InfoNone]
  where
    tp = typeOf name
...
genDropReuse tname scan
  = App (Var (TName nameDropReuse funTp) (InfoExternal [(targetPlatformFromTarget (C CDefault), "drop_reuse(#1,#2,kk_context())")]))
...
genIsUnique tname
  = App (Var (TName nameIsUnique funTp) (InfoExternal [(targetPlatformFromTarget (C CDefault), "is_unique(#1)")]))
        [Var tname InfoNone]
...
genWhitehole tname
  = App (Var (TName nameWhitehole funTp) (InfoExternal [(targetPlatformFromTarget (C CDefault), "kk_block_whitehole(#1)")]))
        [Var tname InfoNone]
...
genConsWhitehole tname
  = App (Var (TName nameWhitehole funTp) (InfoExternal [(targetPlatformFromTarget (C CDefault), "kk_datatype_ptr_whitehole(#1,kk_context())")]))
        [Var tname InfoNone]
  where funTp = TFun [(nameNil, typeOf tname)] typeTotal typeUnit
-}
...
genFree :: TName -> Expr
genFree tname
  = App (Var (TName nameFree funTp) (InfoExternal [(targetPlatformFromTarget (C CDefault), "kk_constructor_free(#1,kk_context())")]))
        [Var tname InfoNone]
  where funTp = TFun [(nameNil, typeOf tname)] typeTotal typeUnit
...
genReuseDrop tname
  = App (Var (TName nameReuseDrop funTp) (InfoExternal [(targetPlatformFromTarget (C CDefault), "kk_reuse_drop(#1,kk_context())")]))
        [Var tname InfoNone]
...
genReuseNull :: Expr
genReuseNull
  = App (Var (TName nameReuseNull funTp) (InfoExternal [(targetPlatformFromTarget (C CDefault), "kk_reuse_null")])) []
  where funTp = TFun [] typeTotal typeReuse

-- Generate a reuse a block
genReuseAddress :: TName -> Expr
genReuseAddress tname
  = App (Var (TName nameReuse funTp) (InfoExternal [(targetPlatformFromTarget (C CDefault), "reuse_datatype(#1,kk_context())")])) [Var tname InfoNone]
  where
    tp    = typeOf tname
...
genReuseAssignWith reuseName arg
  = let assign = TName nameAssignReuse (TFun [(nameNil,typeReuse),(nameNil,typeReuse)] typeTotal typeUnit)
    in App (Var assign (InfoExternal [(targetPlatformFromTarget (C CDefault), "#1 = #2")])) [Var reuseName InfoNone, arg]
