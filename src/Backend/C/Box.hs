                ...
                then -- function match
                     case pat of
                       PatVar tname PatWild
                         -> -- ok, no nested match
                            do i <- unique
                               let uname = newHiddenNameEx "fun-unbox"  (show i)
                     ...
                     do i <- unique
                        let uname = newHiddenNameEx "box" (show i)
...
boxVarAtTp tp
  = Var (TName nameBox tp) (InfoExternal "box(#1)")
unboxVarAtTp tp
  = Var (TName nameUnbox tp) (InfoExternal "unbox(#1)")
