    ...
    case expr of
     ...
     Let groups body
       -> do decls1       <- genGroups False groups
             (doc) <- genExpr body
             return $ (notImplemented $ text "defs") -- (decls1 <-> decls2, doc)
...
genPure expr
  = case expr of
     ...
     Var name info
       -> return $ asVar name -- genTName name
...
asVar :: TName -> Doc
asVar n = var (str $ show $ getName n) (transformType $ tnameType n)
