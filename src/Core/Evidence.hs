      ...
      Lam params eff body
        -> let params' = map param params
           in do (evCtx', body') <- evExpr pnil body
                 let evs = map toParam (evFromEffectType eff) -- FIXME TODO: The parameters should be constructed from evCtx'.
...
evFromEffectType eff
  = let (constants, tail) = extractOrderedEffect eff
        labels            = map (runtimeLabelName . labelName) constants
...
runtimeLabelName :: Name -> Name
runtimeLabelName lbl = lbl -- FIXME TODO: match whatever names are generated by the parser.
...
evClosureR p (l : q) expr
  = case lookup l p of
      Nothing
        -> let (p', expr') = evClosureR p q expr
               p''         = (l, Ev l) : p'
               x           = undefined -- FIXME TODO: construct a term variable whose type is ev l.
               expr''      = App expr' [x]
           in (p'', expr'')
      Just (Ev l)
        -> let (p', expr') = evClosureR p q expr
