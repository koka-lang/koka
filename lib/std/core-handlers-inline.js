...
const Eff = (function() {
  ...
  function yield_op( effect_tag, op_tag, op_arg ) {
    ...
    return _find_handler(effect_tag, op_tag, op_arg, _yield_op_found);
  }

  function _yield_op_found( handler, branch, skip, op_arg) {
    if (branch.resume_kind !== _resume_kind.Tail) {
      ...
      return { 
        ...
        handler   : handler,            // to handler to where to yield
        branch    : branch,             // the handler branch that matches our operation      
      }
    }
    else if (skip == 1) {
      return _skip1_handle( handler, branch, op_arg );
    }
    else {
      ...
      const skiphandler = { effect_tag: _effect_skip, skip: skip, handler: handler };  // todo: avoid this allocation with special skip frames?
      return _skip_handle(skiphandler, branch.branch_fun, op_arg );
    }
  }
  ...
  function _skip1_handle( handler, branch, op_arg ) {
    _handlers_pop(handler);
    const x = _handle_tailcalls( branch.branch_fun(_tail_resume, op_arg, handler.local) );
    return _skip1_handle_yield(handler,x);
  }
  function _skip1_resume( handler, cont, arg ) {
    _handlers_pop(handler);
    const x = _handle_tailcalls( cont(arg) );
    return _skip1_handle_yield(handler,x);
  }
  function _skip1_handle_yield(handler, x) {
    if (_yielding(x)) {
      // extend the continuation with the skip frame 
      _assert(!_tailcalling(x), "_skip1_handle_yield: tail calls should already have been handled.");
      if (x.branch.resume_kind !== _resume_kind.Never) {
        const cont = x.cont; // capture locally
        x.cont = function(arg) {
          _handlers_push(handler);
          return _skip1_resume( handler, cont, arg );
        }
      }
      // and reyield
      return x;
  ...
  function _find_handler( effect_tag, op_tag, op_arg, next ) {
    ...
    for(var i = handlers.length - 1; i >= 0; i--) {
      ...
      if (skipping > 0) {
        ...
        for(var j = 0; j < h.branches.length; j++) {
          ...
          let b = h.branches[j];
          if (b.op_tag == op_tag) {
            return next(h, b, handlers.length - i, op_arg);
          }
        }
  ...
  function _tail_resume_swap( newlocal, result ) {
    return { _result: result, local: newlocal };
  }

  function new_branch1( resume_kind, op_tag, branch_fun ) {
    if (resume_kind==_resume_kind.Tail) {
      function branch_fun_swap( resume, op_arg, local) {
        return branch_fun(_tail_resume_swap, op_arg, local );
      }
      return new_branch(resume_kind,op_tag,branch_fun_swap);
    }
    else {
      function branch_fun_swap( resume, op_arg, local ) {
        function resume_swap(newlocal,result) {
          return resume(result,newlocal);
        }
        return branch_fun( resume_swap, op_arg, local );
      }
      return new_branch(resume_kind,op_tag,branch_fun_swap);
    }
  }
