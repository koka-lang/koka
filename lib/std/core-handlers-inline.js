...
// Branches that are
const _resume_kind = {
  Never      : 0,    // never calls `resume` or `resume-shallow`.
  // the next three are only for deep resume
  ...
  // the following are also used if `resume-shallow` is called.
                     ...
                     branches, skip, handler_kind, handler_tag)
{
  ...
  this.inject_level = 0;            // Only used by inject handlers
}
...
function with_local(init,body) {
  const r = { value: init };
  return _bind_local_ref(r,body(r));
}

function _bind_local_ref(r,x) {
  if (x instanceof Yield) {
    const cont = x.cont; // capture locally
    const val = r.value; // capture current value of the reference
    x.cont = function(arg) {
      r.value = val;     // restore reference value
      return _bind_local_ref(r,cont(arg));
    }
    return x;
  }
  else {
...
function _handlers_find( effect_name, handler_tag, op_name ) {
  ...
  for(i = n - 1; i >= 0; i--) {
        ...
        (handler_tag === 0 || hinfo.handler_tag === handler_tag))
    {
      ...
      if (hinfo.handler_kind===_handler_kind._Inject) { // Inject frame
         if (hinfo.inject_level <= inject) {  // inject handler for our level?
           inject++;   // inject effect
         }
         // otherwise ignore this frame
      }
      else if (inject > 0) {
        inject--;     // skip over this handler due to previous injection
      }
      else return i;
    }
...
function _handle_result(hinfo,result) {
  ...
  while(result instanceof Yield && result.match) {
    ...
    else if (branch.resume_kind <= _resume_kind.Scoped) {
...
const ContextNever = (function() {
  ...
  ContextNever.prototype.resume_shallow = function(arg) {
    throw new Error("A handler branch marked as never-resuming tried to shallow resume.");
  }
  return ContextNever;
})();
...
const ContextTail = (function() {
  ...
  ContextTail.prototype.resume_shallow = function(arg) {
    //this.invoked = true;
    //return this.cont(arg);
    throw new Error("Branch marked as tail-resumptive tried to shallow resume");
  }
  return ContextTail;
})();
...
const ContextNormal = (function() {
  ...
  ContextNormal.prototype.resume_shallow = function(arg) {
    this.invoked = true;
    return this.cont(arg);
  }
...
const ContextSkipTail = (function(){
  ...
  ContextSkipTail.prototype.resume_shallow = function(arg) {
    throw new Error("Branch marked as tail-resumptive tried to shallow resume (from a skip frame)");
  }
...
function handle_inject( effect_name, handler_tag, inject_level, action ) {
  var hinfo = new HandlerInfo(effect_name,null,null,null,[],0,_handler_kind._Inject,handler_tag);
  hinfo.inject_level = inject_level;
  const inject = new Handler( hinfo );
...
function handle_inject_exn( inject_level, action ) {
  return _resume_inject_exn(inject_level, action, undefined);
}

function _resume_inject_exn(inject_level, cont, arg ) {
  try {
    const result = cont(arg);
    if (result instanceof Yield) {
      const cont = result.cont; // capture locally
      result.cont = function(arg) {
        return _resume_inject_exn(inject_level, cont, arg);  // resume within the try block
      };
    }
    return result;
  }
    ...
    if (exn instanceof SystemException) {
      ...
      if (exn._inject == null) { exn._inject = 0 }

      // increase injection level if we are meant to catch it
      if (inject_level <= exn._inject) {
        exn._inject++;
      }
      throw exn;  // rethrow
    }
