...
function _new_skip_frame(skip) {
  return new Handler("<skip>",id,[],undefined,skip,_handler_kind._Skip,0);
}
...
function finalize(resume, result) {
  if (resume===_resume_tail_return) {
    ...
    _handle_catch(
      ...
      function(exn) { if (exn===fexn) return result; else throw exn; },
...
function new_handler_linear( effect_name, return_fun, branches ) {
  return new_handler(effect_name,return_fun,branches,_handler_kind.Linear);
}

function new_handler_shallow( effect_name, return_fun, branches ) {
  return new_handler(effect_name,return_fun,branches,_handler_kind.Shallow);
}

var _handler_tags = 1;

function Handler(effect_name, return_fun, branches, local, skip, handler_kind, handler_tag) {
...
// Create a handler for a given effect
function _new_handlerx( effect_name, return_fun, branches0, handler_kind, handler_tag, wrap_handler_tag)
{
  ...
  const shared_handler = new Handler(effect_name, return_fun, branches, undefined, 0, handler_kind, handler_tag);

  // create a special return operation branch to stay well-typed in `_handle_op`
  const return_branch = (return_fun==null ? null :
                          new_branch_never( null, function(resume,op_arg,local) { return return_fun(op_arg,local); }) );
  ...
  return (function(action,local) {
    const handler = (local === undefined ? shared_handler :
                      new Handler(effect_name, return_fun, branches, local, 0, handler_kind, handler_tag));
    var baction = (return_branch==null ? action :  function(htag) {
                                                      return bind(action(htag), function(x) {
                                                        return new Yield(handler, return_branch, x);
                                                      });
                                                    });
    return _handle_action(handler, function() {
...
function new_handler( effect_name, return_fun, branches, handler_kind ) {
  return _new_handlerx( effect_name, return_fun, branches, handler_kind, 0, null );
}

function new_handler1( effect_name, return_fun, branches, handler_kind ) {
  const h = new_handler(effect_name, return_fun, branches, handler_kind);
  return (function(local,action) { return h(action,local); });
}
function new_handler1_linear( effect_name, return_fun, branches ) {
  return new_handler1(effect_name, return_fun, branches, _handler_kind.Linear);
}
function new_handler1_shallow( effect_name, return_fun, branches ) {
  return new_handler1(effect_name, return_fun, branches, _handler_kind.Shallow);
}
function new_resource_handler( effect_name, return_fun, branches, handler_kind, handler_tag, wrap_handler_tag ) {
  return _new_handlerx( effect_name, return_fun, branches, handler_kind, handler_tag, wrap_handler_tag );
}
function new_resource_handler1( effect_name, return_fun, branches, handler_kind, handler_tag, wrap_handler_tag ) {
  const h = new_resource_handler(effect_name, return_fun, branches, handler_kind, handler_tag, wrap_handler_tag );
  return (function(local,action) { return h(action,local); });
}
...
function _handle_cont(handler,cont,arg) {
  ...
  try {
    ...
    else if (exn instanceof JumpToHandlerException && exn.handler === handler) {
      return exn.result;
    }
    else return new YieldException(exn); // later, rethrow through todo's
  }
  finally {
...
function _handle_op(handler,result,todo_readonly) {
  ...
  while(true) {
    if (result instanceof Yield)
    {
      ...
      if (todo===todo_readonly) todo = todo_readonly.slice();  // copy on write
      const cont = todo.pop();
      result = cont(result);  // note: this will also propegate exceptions through todo`s
    }
...
function handle_inject( effect_name, handler_tag, action ) {
  const inject = new Handler(effect_name,id,[],undefined,0,_handler_kind._Inject,handler_tag);
