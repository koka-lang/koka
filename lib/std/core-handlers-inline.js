...
function _new_skip_frame(skip) {
  return new Handler("<skip>",null,[],undefined,skip,_handler_kind._Skip,0);
}
...
function Handler(effect_name, reinit, branches, local, skip, handler_kind, handler_tag) {
...
function _new_handlerx( effect_name, reinit, return_fun, finally_fun, branches0, handler_kind, handler_tag, wrap_handler_tag)
{
  ...
  const shared_handler = new Handler(effect_name, reinit, branches, undefined, 0, handler_kind, handler_tag);
  ...
  return (function(action,local) {
    ...
    const handler = (local === undefined ? shared_handler :
                      new Handler(effect_name, reinit, branches, local, 0, handler_kind, handler_tag));
     // finally clauses are treated as regular `todo` continuations
    const todo = (finally_fun==null ? [] : [
	                function(result){ 
                    // don't finalize on long jump exceptions
                    if (result instanceof YieldException && result.exn instanceof JumpToHandlerException) return result;
                    return bind_const( finally_fun(handler.local), result); 
                  }]);
    ...
    return _handle_action(handler, haction, todo);
  });
}
...
function _handle_action( handler, action, todo ) {
  return _handle_op( handler, _handle_cont(handler, action, undefined), todo );
}
...
function handle_inject( effect_name, handler_tag, action ) {
  const inject = new Handler(effect_name,null,[],undefined,0,_handler_kind._Inject,handler_tag);
