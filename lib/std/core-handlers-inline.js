...
function yield_op( effect_tag, op_name, op_arg, op_tag ) {
  ...
  if (i<0) throw new Error("there is no handler found for operation " + effect_tag + "/" + op_name)
  const skip   = n - i;
  const branch = handler.branches[op_tag];
  if (branch==null) throw new Error("bad handler: operation " + op_name + " is not found in the handler for " + effect_tag);
...
function finalize(resume, result) {
  if (resume===_resume_tail_return) {  
    ...
    _handle_catch( 
      function() { return resume_throw(resume,fexn,undefined /* local state */); },
...
function new_handler_linear( effect_tag, return_fun, branches ) {
  return new_handler(effect_tag,return_fun,branches,_handler_kind.Linear);
}

function new_handler_shallow( effect_tag, return_fun, branches ) {
  return new_handler(effect_tag,return_fun,branches,_handler_kind.Shallow);
}

function Handler(effect_tag, return_fun, branches, local, skip, handler_kind) {
...
function new_handler( effect_tag, return_fun, branches0, handler_kind ) 
{
  ...
  const shared_handler = new Handler(effect_tag, return_fun, branches, handler_kind);

  // create a special return operation branch to lift in the end.
  ...
  return (function(action,local) {
    const handler = (local === undefined ? shared_handler : 
                      new Handler(effect_tag, return_fun, branches, local, 0, handler_kind)); 
    var baction = (return_branch==null ? action :  function() {
                                                      return bind(action(), function(x) {
                                                        return new Yield(handler, return_branch, x);
                                                      });
                                                    });
    return _handle_action(handler, function() { 
      return yield_iter( baction() ); // autoconvert from iterators
    }); 
  });
}

function new_handler1( effect_tag, return_fun, branches, handler_kind ) {
  const h = new_handler(effect_tag, return_fun, branches, handler_kind);
  return (function(local,action) { return h(action,local); });
}
function new_handler1_linear( effect_tag, return_fun, branches ) {
  return new_handler1(effect_tag, return_fun, branches, _handler_kind.Linear);
}
function new_handler1_shallow( effect_tag, return_fun, branches ) {
  return new_handler1(effect_tag, return_fun, branches, _handler_kind.Shallow);
}


// Run action under a handler
function _handle_action( handler, action ) {
  return _handle_op( handler, _handle_cont(handler, action, undefined /*arg*/), [] );
}
    ...
    if (result instanceof Yield)
    {
      if (result.handler !== handler) {
        ...
        if (branch.resume_kind===_resume_kind.Never) {
          ...
          // `resume` is yielded as a special operation; todo: don't do this for shallow resumptions?
...
function _handle_finally_resume( cont, arg, on_final, reinit) {
  ...
  if (result instanceof Yield) {
    ...
    result.cont = function(arg) {
      ...
      return _handle_finally_resume(c, arg, on_exn, reinit);  // resume within the try block
    };
    return result;
  }   
  else {
