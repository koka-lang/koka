...
function _log_handlers(msg) {
  console.log(msg)
  console.log(" $htop=" + $htop.toString());
  console.log(" $hstack=[\n  " + $hstack.map(_show_handler).join(",\n  ") + "]\n")
}
...
function _asyncStartSynchronous() {
  _syncNesting++;
  if (_syncBlocked===null) _syncBlocked = [];
}

function _asyncEndSynchronous() {
...
// and only the last call back is queued.
function _asyncCallback( cb, haserr, nodup ) {
  ...
  return (function() {
    ...
    if (!self_in_synchronous && _syncNesting > 0) { // if so, put ourselves on the queue
      ...
      if (args) { _syncBlocked.push( { "fun": self, "args": args } ); }
      return;
    }
    else {
...
function _asyncAll( actions, k ) {
  ...
  for( var i = 0; i < n; i++) {
    (function(j){ 
      var cb = _asyncCallback( function(_) {
        return _primcatch( function(k1) {
            return actions[j](k1);
          },
          function(_exn,_k2) {
            if (exn===undefined) exn = _exn;
            return allk(j,undefined,async_yield);
          },
          function(x) { 
