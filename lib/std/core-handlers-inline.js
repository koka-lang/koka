...
function Yield( handler, branch, op_arg, /* optional: */ local, match ) {
  this.cont    = _cont_id;
  this.op_arg  = op_arg;
  this.handler = handler;
  this.branch  = branch;
  this.local   = local;
  this.match   = (match ? true : false);
}
...
const CancelException = (function() {
  ...
  CancelException.prototype.get_result = function() {
    if (this._result instanceof ExceptionValue) throw this._result.exn;
    return this._result;
  }
                        ...
                        branches0, handler_kind, handler_tag, wrap_handler_tag)
{
  ...
  const hinfo = new HandlerInfo(effect_name, reinit_fun, return_fun, finally_fun,
...
function _handle_resume( hinfo, local, cont, arg, todo ) {
  const handler = _handlers_push_hinfo(hinfo,local);
  return _handle_result( hinfo, _handle_cont_pushed(handler, cont, arg), todo );
}

function _handle_resume_pushed( handler, cont, arg, todo ) {
  return _handle_result( handler.info, _handle_cont_pushed(handler, cont, arg), todo );
}
...
function _handle_cont_pushed(handler,cont,arg) {
  ...
  try {
    ...
    const finally_fun = (hinfo.finally_fun || _cont_id);
      ...
      if (exn instanceof CancelException && exn.handler === handler) {
        return exn.get_result();  // might throw
      }
      else {
        throw exn;
      }
  ...
  if (result instanceof Yield) {
    // hack: communicate local and handler match through the yield result..
    result.local = local;
    result.match = (result.handler === handler);
    return result;
  }
...
function _handle_result(hinfo,result,todo_readonly) {
  var todo = todo_readonly;
  ...
  while(true) {
    if (result instanceof Yield) {
      if (result.handler===null) {
        ...
        const handler = _handlers_push_hinfo(hinfo,result.local);
  			result = result.op_arg(handler); // the operation argument contains the resume function itself
      }
        ...
        result.cont = function(arg) {
          ...
          if (resume_count > 1 && hinfo.reinit_fun != null) {
            return bind( hinfo.reinit_fun(local), function(newlocal) {
              return _handle_resume( hinfo, newlocal, cont, arg, todo);
            });
          }
          else {
            return _handle_resume( hinfo, local, cont, arg, todo );
          }
        }
        ...
        if (branch.resume_kind===_resume_kind.Never) {
          ...
          context = new ContextTail(hinfo,cont);
        }
        else {
          // `resume` directly, will use more stack space since it calls `_handle_op` again
          context = new ContextNormal(hinfo,cont,todo);
        }
        result = _branch_invoke( branch, context, result.op_arg, local);
      }
    }
...
const ContextTail = (function() {
  ...
  function ContextTail(hinfo,cont) {
    Context.call(this);
    this.hinfo = hinfo;
    this.cont = cont;
  }
  ...
  ContextTail.prototype.resume = function(arg,local) {
    this.invoked = true;
    function resume_cont(handler) {
      _assert(handler.info === this.hinfo, "Different handler resumes in tail resumption");
      // don't use `handle_result` as we resume inside the trampoline of `handle_result`
      return _handle_cont_pushed( handler, this.cont, arg );
    }
    return new Yield( null /* handler */, null /*branch*/, resume_cont /*op_arg*/, local, true /* match */);
  }
  ContextTail.prototype._resume_cancel = function(arg) {
    this.invoked = true;
    function resume_cont(handler) {
      _assert(handler.info === this.hinfo, "Different handler resumes in tail finalization");
      return _handle_cont_pushed( handler, this.cont,
                                  new ExceptionValue( new CancelException(handler,arg) ) );
    }
    return new Yield( null /* handler */, null /*branch*/, resume_cont /*op_arg*/, undefined, true /* match */);
  }
  return ContextTail;
})();
...
const ContextNormal = (function() {
  ...
  function ContextNormal(hinfo,cont,todo) {
    Context.call(this);
    this.hinfo = hinfo;
    this.cont = cont;
    this.todo = todo;
  ...
  ContextNormal.prototype.resume = function(arg,local) {
    this.invoked = true;
    const handler = _handlers_push_hinfo(this.hinfo, local);
    return _handle_resume_pushed( handler, this.cont, arg, this.todo);
  }
...
function _resume_inject( inject, cont, arg ) {
  ...
  try {
    ...
    // rule (cancel_i), fig 9.
    if (exn instanceof CancelException && exn.handler===inject) {
