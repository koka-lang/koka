...
function yield_op( effect_tag, op_name, op_arg, op_tag ) {
  ...
  if (is_linear || (branch.resume_kind === _resume_kind.Tail && _resume_tail_count < _max_resume_tails)) {
    ...
    try {
      ...
      throw new JumpToHandlerException( handler, null, exn );
    }
    finally {
...
function finalize(resume, result) {
  if (resume===_resume_tail_return) {  
    ...
    _handle_catch( 
      function() { return resume(undefined,undefined,fexn); },
...
function _skip_handle_yield( handler, res, skipframe ) {
  ...
  if (res instanceof _TailResume) {
    ...
    throw new JumpToHandlerException(handler, res, null);
  }
}
...
function JumpToHandlerException( handler, result, exn ) {
  this.handler = handler;
  this.result = result;
  this.exn = exn;
...
function new_handler( effect_tag, return_fun, branches0, is_linear ) 
{
  ...
  return (function(action,local) {
    ...
    var baction = (return_fun==null ? action :  function() {
                                                  return bind(action(), function(x) {
                                                    return return_fun(x,handler.local);
                                                  });
                                                });
...
function new_handler1( effect_tag, return_fun, branches, is_linear ) {
  const h = new_handler(effect_tag, return_fun, branches, is_linear);
...
function _handle_action( handler, action ) {
  return _handle_yield( _resume_cont(handler, action, undefined, undefined), handler);
}
...
function _resume_cont( handler, cont, arg, exn ) {
  ...
  try {
    ...
    if (exn instanceof FinalizeToHandlerException && exn.handler === handler) {
      ...
      if (exn.exn != null) throw exn.exn;
      return exn.result;
    }
    else throw exn; // rethrow
  }
  finally {
...
function _call_branch(handler, branch, cont, op_arg, local, resumed ) {
  if (branch.resume_kind === _resume_kind.Tail) {
    return branch.branch_fun( function(arg,local,exn) {
                                resumed.resumed = true;
                                if (local !== undefined) handler.local = local;
                                return _resume_cont(handler,cont,arg,exn); // and repeat the while(tailresumed)
                              }, op_arg, local);
  }
...
function handle_catch( action, on_exn, catchall) {
  return _handle_catch_resume( action, undefined, undefined, on_exn, (catchall ? true : false));
}

function handle_finally( action, on_final, reinit) {
  return _handle_finally_resume( action, undefined, undefined, on_final, reinit);
}

function _handle_catch_resume( cont, arg, argexn, on_exn, catchall) {
  try {
    const result = cont(arg,argexn);
    if (result instanceof Yield) {
      const cont = result.cont; // capture locally
      result.cont = function(arg,argexn) {
        return _handle_catch_resume(cont, arg, argexn, on_exn, catchall);  // resume within the try block
      };
    }   
    return result;
  }
...
function _handle_finally_resume( cont, arg, argexn, on_final, reinit) {
  let result;
  try {
    result = cont(arg,argexn); 
  }
  catch(exn) {
    if (exn instanceof JumpToHandlerException) {
  ...
  if (result instanceof Yield) {
    ...
    result.cont = function(arg,argexn) {
      ...
      if (reinit != null && resumes > 1) {
        c = function(arg,argexn) {  
              return bind(reinit(), function(_x) { return cont(arg,argexn); }); 
            };
      }
      return _handle_finally_resume(c, arg, argexn, on_final, reinit);  // resume within the try block
    };
    return result;
  }   
  else {
