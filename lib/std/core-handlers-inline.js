...
function Handler(effect_name, reinit_fun, return_fun, finally_fun,
                 branches, local, skip, handler_kind, handler_tag)
{
...
// Only CancelExceptions are an instance of this so we might simplify this
// away in the future?
const FinalizeException = (function() {
  __extends(FinalizeException,Error);
...
const CancelException = (function() {
  __extends(CancelException,FinalizeException);
  function CancelException( handler, result ) {
    FinalizeException.call(this);
    this.handler = handler;
    this.result = result;
  }
  CancelException.prototype.match_handler = function(handler) {
    return (this.handler===handler ||
            (this.handler.effect_name === handler.effect_name &&
             this.handler.handler_tag === handler.handler_tag));
  }
  CancelException.prototype.get_result = function() {
    if (this.result instanceof ExceptionValue) throw this.result.exn;
    return this.result;
  }
...
function yield_op( effect_name, op_name, op_arg, op_tag, handler_tag ) {
  ...
  if (is_linear || (branch.resume_kind <= _resume_kind.Tail && _resume_tail_count < _max_resume_tails)) {
    ...
    try {
      ...
      throw new CancelException( handler, new ExceptionValue(exn) );
    }
    finally {
...
function _new_skip_frame(skip) {
  return new Handler("<skip>",null,null,null,[],undefined,skip,_handler_kind._Skip,0);
}
...
function _skip_handle_yield( handler, res, skipframe ) {
  ...
  if (res instanceof _TailResume) {
    ...
    throw new CancelException(handler, res);
  }
}
                        ...
                        branches0, handler_kind, handler_tag, wrap_handler_tag)
{
  ...
  const shared_handler = new Handler(effect_name, reinit_fun, return_fun, finally_fun,
                                     branches, undefined, 0, handler_kind, handler_tag);
  ...
  return (function(action,local) {
    ...
    const handler = (local === undefined ? shared_handler :
                      new Handler(effect_name, reinit_fun, return_fun, finally_fun,
                                  branches, local, 0, handler_kind, handler_tag));

  	// the handler action gets the resource argument (the identifier of the handler)
		const haction = function() {
                      const resource = (wrap_handler_tag==null ? handler.handler_tag :
                                                                 wrap_handler_tag(handler.handler_tag));
...
function _handle_cont(handler,cont,arg) {
  ...
  try {
    ...
    // rule (unwind), fig 9.
		_handlers_pop(handler);  // pop handlers before calling finally
    const finally_fun = (handler.finally_fun || _cont_id);
		return bind( finally_fun(handler.local), function(_res) {
      if (exn instanceof CancelException && exn.match_handler(handler)) {
        return exn.get_result();
      }
      else {
        throw exn;
      }
  ...
  // rule (return), fig 5.
  if (handler.finally_fun != null) {
    return _bind_finally( handler, function(_result){
                                      return handler.return_fun(result,handler.local)
                                    }, result );
  }
  else if (handler.return_fun != null) {
    return handler.return_fun(result,handler.local);
  }
  else {
    return result;
    ...
    if (result instanceof Yield)
    {
      if (result.handler !== handler) {
        ...
        result.cont = function(arg) {
          ...
          if (resume_count > 1 && handler.reinit_fun != null) {
            return bind( handler.reinit_fun(handler.local), function(newlocal) {
        ...
        // exit the trampoline for now
        return result;
      }
      else if (result.branch===null) {
        // a resume is yielded as a special operation to trampoline them
        if (result.cont !== _cont_id) {  // tail-resumptive operations have a cont_id so no need to push onto the todo list
        ...
        // invoke the resumption
				// note: no need for try since deep handlers will invoke under
				// the handler again ensuring finally clauses are executed on exceptions
        result = result.op_arg(); // the operation argument contains the resume function itself
      }
      else {
        ...
        if (branch.resume_kind===_resume_kind.Never) {
          ...
          context = new ContextTail(handler,cont);
        }
        else {
          // `resume` directly, will use more stack space since it calls `_handle_op` again
          context = new ContextNormal(handler,cont,todo);
        }
        result = _branch_invoke( branch, context, result.op_arg, handler.local);
      }
    }
...
function _bind_finally( handler, f, x ) {
  // section 8.4: return as an operation
	...
	try {
		...
		if (result instanceof Yield) {
			const cont = result.cont;
			result.cont = function(arg) {
				return _bind_finally( handler, cont, arg );
			}
			return result;
		}
	}
	...
	return bind_const(handler.finally_fun(handler.local), result );
}
...
function _protect( context, cont, arg ) {
	try {
		var result = cont(arg);
		if (result instanceof Yield) {
			if (result.branch != null /* not tail-resume */ && !context.invoked) {
				// only extend the continuation if this is not
				// a tail resuming yield (`ContextTail`) and the context is not already invoked
...
const Context = (function() {
  ...
  Context.prototype.auto_finalize = function(arg) {
    return (this._suppress || this.invoked ? arg : this.finalize(arg));
  }
...
const ContextTail = (function() {
  ...
  function ContextTail(handler,cont) {
    Context.call(this);
    this.handler = handler;
    this.cont = cont;
  }
  ...
  // of each branch uses `bind` translation if the resume was not in a tail-call
  // position.
  ContextTail.prototype.resume = function(arg,local) {
    ...
    function resume_cont() {
      ...
      if (local !== undefined) this.handler.local = local;
      return this.cont(arg);
    }
    return new Yield(this.handler, null /*branch*/, resume_cont /*op_arg*/);
  }
  ContextTail.prototype._resume_cancel = function(arg) {
    return this.resume( new ExceptionValue( new CancelException(this.handler,arg) ), undefined /* local */ );
  }
  return ContextTail;
})();
...
const ContextNormal = (function() {
  ...
  function ContextNormal(handler,cont,todo) {
    Context.call(this);
    this.handler = handler;
    this.cont = cont;
    this.todo = todo;
  ...
  ContextNormal.prototype.resume = function(arg,local) {
    this.invoked = true;
    if (local !== undefined) this.handler.local = local;
    return _handle_op(this.handler,this.cont(arg),this.todo);
  }
...
function handle_inject( effect_name, handler_tag, action ) {
  const inject = new Handler(effect_name,null,null,null,[],undefined,0,_handler_kind._Inject,handler_tag);
...
function _resume_inject( inject, cont, arg ) {
  ...
  try {
    ...
    // rule (cancel_i), fig 9.
    if (exn instanceof CancelException && exn.match_handler(inject)) {
      // wrap the exn inside another CancelException to keep propagating beyond the inject
      throw new CancelException(inject, new ExceptionValue(exn));
    }
    else {
...
function _handle_catch_resume( cont, arg, on_exn, catchall) {
  try {
    ...
    if (!catchall && exn instanceof FinalizeException) {
