...
function _handlers_find( effect_name, handler_tag, op_name ) {
  ...
  throw new Error("there is no handler found for operation " + _show_effect(effect_name,handler_tag) + "/" + op_name);
}
...
function new_handler( effect_name, return_fun, branches0, handler_kind )
{
  ...
  const shared_handler = new Handler(effect_name, return_fun, branches, handler_kind);
  ...
  return (function(action,local) {
    const handler = (local === undefined ? shared_handler :
                      new Handler(effect_name, return_fun, branches, local, 0, handler_kind));
    ...
    return _handle_action(handler, function() {
      return yield_iter( baction(handler.handler_tag) ); // autoconvert from iterators
    });
  });
}
