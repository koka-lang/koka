...
function bind( x, next ) {
  if (x instanceof Yield) {
    _bind_cont(x,next);  // separate function for inlining; makes bind 25% faster on Node v8.8
    return x;
  }
  else {
    return next(x);
...
function yield_op( effect_tag, op_tag, op_idx, op_arg ) {
  ...
  const skip   = n - i;
  const branch = handler.branches[(op_idx >= 0 ? op_idx : op_tag)];
