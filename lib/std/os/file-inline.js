...
var _file_stat = function(fname,cb) { notimp("stat",fname); };
...
if ($std_core.host()=="node") 
{
  ...
  _file_download_text = function(urlstr, timeout, cb) {
    var cbfun = $std_core._async_callback_err1(cb); // set up first as it captures the handler stack!
    var t = null;
    var timedOut = false;
    // start download
    var req = _download_text(urlstr, timeout, function(err,data) {
      if (timeout > 0 && t) {
        clearTimeout(t);      
      }
      if (err && timedOut) return cbfun(new Error("server is not responsive.\n  url: " + urlstr),data)
                      else return cbfun(err,data);
    });
    // set timeout
    if (timeout > 0) {
  ...
  function _download_text(urlstr, timeout, cb, redirected) {
    ...
    function timeoutReset(msecs) { 
      timeoutClear();
      if (timeout) { t = setTimeout(timeoutAbort, msecs); }
    }
    // Issue the request
