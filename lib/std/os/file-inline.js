...
if ($std_core.host()=="node")
{
  ...
  _file_download_text = function(urlstr, timeout, cb) {
    ...
    var req = _download_text(urlstr, (timeout > 0), function(err,data) {
      ...
      if (err) return cb(new Error("server is not responsive.\n  url  : " + urlstr + "\n  error: " + err),data)
      else if (timedOut || req.aborted) return cb(new Error("server is not responsive.\n  url: " + urlstr),data)
  ...
  function _download_text(urlstr, use_timeout, cb, redirected) {
    // Check that there is progress (some response every 5s) (do this only if timeout is set)
    var progress = 5000;
    var req = null;
    var t   = null;
      ...
      switch(response.statusCode) {
        case 200:
          ...
          response.on('data', function(chunk){
            ...
            timeoutClear();
            //console.log(" download complete with error: " + err);
        ...
        case 307:
          ...
          return _download_text(response.headers.location, use_timeout, cb, true );
          break;
        default:
          timeoutClear();
