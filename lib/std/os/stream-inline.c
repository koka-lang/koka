...
static kk_std_os_uv__uvStatusCode kk_uv_read_stop(kk_std_os_stream__uvStream stream, kk_context_t* _ctx){
  return kk_std_os_uv_int_fs_status_code(uv_read_stop((uv_stream_t*)stream.internal), _ctx);
}
...
static void kk_uv_write(kk_std_os_stream__uvStream stream, kk_std_core_types__list buffs, kk_function_t cb, kk_context_t* _ctx){
...
static void kk_uv_write2(kk_std_os_stream__uvStream handle, kk_std_core_types__list buffs, kk_std_os_stream__uvStream send_handle, kk_function_t cb, kk_context_t* _ctx){
  uv_write_t* write = kk_malloc(sizeof(uv_write_t), _ctx);
  kk_uv_callback_t* wrapper = kk_malloc(sizeof(kk_uv_callback_t), _ctx);
  wrapper->callback = cb;
  write->data = wrapper;
...
static int32_t kk_uv_try_write(kk_std_os_stream__uvStream stream, kk_std_core_types__list buffs, kk_context_t* _ctx){
  int list_len;
  const uv_buf_t* uv_buffs = kk_bytes_list_to_uv_buffs(buffs, &list_len, _ctx);
  return uv_try_write((uv_stream_t*)stream.internal, uv_buffs, list_len);
}

static int32_t kk_uv_try_write2(kk_std_os_stream__uvStream handle, kk_std_core_types__list buffs, kk_std_os_stream__uvStream send_handle, kk_context_t* _ctx){
  int list_len;
  const uv_buf_t* uv_buffs = kk_bytes_list_to_uv_buffs(buffs, &list_len, _ctx);
...
static kk_std_os_uv__uvStatusCode kk_uv_stream_set_blocking(kk_std_os_stream__uvStream stream, bool blocking, kk_context_t* _ctx){
  return kk_std_os_uv_int_fs_status_code(uv_stream_set_blocking((uv_stream_t*)stream.internal, blocking), _ctx);
}
