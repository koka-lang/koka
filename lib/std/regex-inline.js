...
function $regex_nong( r ) {
  ...
  r.regexng = new RegExp( r.source, r.flags )
  return r.regexng;
}
...
function $regexExec( r,  s, start ) 
{
  ...
  if (!match) {
    ...
    return $std_core.Just(Matched(match.index, next, match[0] ? match[0] : "", groups));
  }
}  
...
function $regexExecAll( r,  s, start ) 
{
  ...
  while (match = r.regex.exec(s)) {
    ...
    var groups = (r.offsets ? $regexGroups(r,match) : match);
    result.push( Matched( match.index, r.regex.lastIndex, match[0] ? match[0] : "", groups ) );
  }
  return result;
}  
...
{
  var regex = (all === 0 ? $regex-nong(r) : r.regex);
  ...
  return s.replace( regex, function() {
    ...
    var groups = (r.offsets ? $regexGroups(r,match) : match);
    return repl( Matched( index, next, matched, groups) );      
  });
}
