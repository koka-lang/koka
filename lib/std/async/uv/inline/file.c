...
static void kk_uv_file_open_callback(uv_fs_t* req) {
  kk_uv_req_callback((uv_req_t*)req, &kk_uv_fs_result_call);  
}

kk_std_core_exn__error kk_uv_file_open_setup(kk_uv_loop_t loop, kk_string_t fpath, int32_t flags, int32_t mode, kk_function_t cb, kk_context_t* ctx) {
  uv_fs_t* req; 
  ...
  kk_with_string_as_qutf8_borrow(fpath,cfpath,ctx) {
    err = uv_fs_open(kk_uv_loop(loop,ctx), req, cfpath, flags & ~0x3, flags & 0x3, &kk_uv_file_open_callback);
  }
...
static void kk_uv_file_close_callback(uv_fs_t* req) {
  kk_uv_req_callback((uv_req_t*)req, &kk_uv_fs_result_call);  
}

kk_std_core_exn__error kk_uv_file_close(kk_uv_loop_t loop, kk_ssize_t fhandle, kk_function_t cb, kk_context_t* ctx ) {
  uv_fs_t* req; 
  int err = kk_uv_req_create(sizeof(uv_fs_t), cb, (uv_req_t**)&req, ctx );
  if (err!=0) return kk_error_from_uv_errno(err,ctx);
  err = uv_fs_close(kk_uv_loop(loop,ctx), req, fhandle, &kk_uv_file_close_callback); 
  if (err!=0) { kk_uv_req_free((uv_req_t*)req,ctx); return kk_error_from_uv_errno(err,ctx); }
  return kk_result_uv_req_dispose0((uv_req_t*)req,ctx);
} 
