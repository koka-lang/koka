...
function kk_fd_open_setup( evloop, path, flags, mode, cb ) {
  ...
  fs.open(path,flags,mode,(err,fd) => cb(kk_result(err,fd)));
  return kk_result_ok_nodispose();
}

function kk_fd_close_setup( evloop, fd, cb ) {
  if (!fs) return kk_enotsupp();
  fs.close(fd,(err) => cb(kk_result_unit(err)));
  return kk_result_ok_nodispose();
}

function kk_fd_read_setup( evloop, fd, bytes, ofs, cb ) {
  if (!fs) return kk_enotsupp();
  var buf = Buffer.from( bytes.buffer );
  fs.read(fd,{ buffer: buf, position: ofs }, (err,nread,_buf) => cb(kk_result(err,nread)));
  return kk_result_ok_nodispose();
}
