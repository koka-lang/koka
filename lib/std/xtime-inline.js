...
function _now() {
  ...
  var d      = Date.now();
  var udays  = Math.floor(d / msecs_per_day);
  var msecs  = d - (udays*msecs_per_day);
  var secs   = Math.floor(msecs/1000);
  var nsecs  = (msecs - (1000*secs)) * 1e6;
  return Instant($std_core._int_double(udays),secs,nsecs); 
}

function _now_resolution() {
  return 1e6;
}
