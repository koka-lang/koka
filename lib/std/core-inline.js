...
function exn_info( exn ) {
  ...
  else if (exn instanceof ReferenceError) {
    return Reference;
  }
  else {
    return Error;
...
$std_core = { "_export": _export
            ...
            , "Yield": Yield
            // double operations that will be inlined
            //, "_double_trunc": _double_trunc
...
function _double_round( d ) {
  ...
  var fr = d - fl;
  if (fr < 0.5) return fl;
  else if (fr > 0.5) return fl + 1.0;
  else if ((fl%2.0)===0.0) return fl;
  else return fl + 1.0;
}
