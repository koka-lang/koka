...
function $errorUnhandledOperation(yld) {
  error("Internal: unhandled operation: " + yld.optag );
}
...
var $exnTag = _Eff__hexn()._tag; 
var $asyncTag = _Eff__asyn()._tag; 
...
function $asyncOn( async, cont ) {
  ...
  async_on( async, function(x) {
    $hstack = hstack;
    $htop = htop;
    return $genericOptHandlerExn( cont, x );
  });
  async_on_exn( async, function(exn) {
    $hstack = hstack;
    $htop = htop;
    return $genericOptHandlerExn( function(_) { throw exn; }, _unit_ );
  });
  return Yield(_Eff__asyn(_Op__ayield), function(x){ return $errorNoResume("async"); });
}

function $asyncCallback( k ) {
  var hstack = $hstack;
  var htop = $htop;
  return function(err,x) {
    $hstack = hstack;
    $htop = htop;
    return $genericOptHandlerExn( function(_) {
      if (err) throw err; 
      return k(x);      
    }, _unit_ );
  };
...
function $asyncAll( actions, k ) {
  ...
  function allk(i,x) {
    ...
    if (n <= 0) {
      $hstack = hstack;
      $htop   = htop;
      return $genericOptHandlerExn(function(xs) { return k(xs); },results);
    }
    else {
      return ayield();
    }
  ...
  for( var i = 0; i < n; i++) {
    (function(j){ 
      actions[j]( function(x) { allk(j,x); return ayield(); }); 
...
function $genericOptHandlerExn( cont, x ) {
  while(true) {
    try {
...
function $withGenOptHandler( handler, action, startCps ) {
  if (!startCps) return $withGenOptHandlerX(handler,action);
  
  // startCps: sometimes we are called inside a cps context (with a $hstack)
  // but we are called in a total setting and need start our own fresh $hstack.
  var hstack;
  var htop;
  try {
    hstack  = $hstack; 
    ...
    $hstack = [];
    $htop   = -1;
    return $withGenOptHandlerX(handler,action);
  }
  finally {
    $hstack = hstack;
    $htop = htop;
...
function $withGenOptHandlerX( handler, action ) {
  $hstack = $hstack.slice(0,$htop+1); // shallow copy since some resume may hold it 
  $hstack.push(handler);
  $htop++;    
  return ($htop === 0 ? $genericOptHandlerExn(action,Result) : action(Result)); 
}
