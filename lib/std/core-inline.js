...
function $makeGenOptHandler1(optag,ret,ops) {
  return function(action,loc1,k) {
    ...
    return $withGenHandler( handler, action, k );
  };
}
...
function $withGenHandler( handler, action, k ) {
  var hstack;
  var htop;
  if (k===undefined) {
    hstack = $hstack; htop = $htop;
    $hstack = [];
    $htop = -1;
  }
  ...
  $hstack.push(handler);
  $htop++;    
  var res = ($htop === 0 ? $genericOptHandler(action(Result)) : action(Result));
  if (k===undefined) {
    $hstack = hstack;
    $htop = htop;
  }
