...
function _vector_slice(a, start, end) {
  ...
  if (start < 0 || start > length) {
    throw new Error("Out of bounds start index: " + start);
  }

  if (end < 0 || end > length) {
    throw new Error("Out of bounds end index: " + end);
  }
...
function _vector_zip2(a, b, fn) {
  ...
  while (index < length_a && index < length_b) {
    out.push(fn(a[index], b[index]));
    ++index;
  }
