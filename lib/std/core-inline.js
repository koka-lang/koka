...
function $makeHandler1(optag,ret,ops) {
  ...
  var loc;
  var k;
  function resume(_cont,_x,_loc,_k) { cont = _cont; x = _x; loc = _loc; k = _k; }
  function hndler(_cont,_x,_loc,_k) { cont = _cont; x = _x; loc = _loc; k = _k; 
    var res;
    do {
      var yld = cont(x);
      if (yld._tag === 1) {
        if (yld.op._tag === optag) res = ops(yld.op._field1, yld.cont, resume, loc, k);
                              else res = $reyield1(yld, hndler, loc, k);
      }
      else res = ret(yld.result, loc, k);
    }
    while(res===undefined);
    return res;    
  }

  return function(action,loc,k) {
    if (k===undefined) k = id;
    return hndler(action,Result,loc,k);
  };
}
...
var $handlersId  = 0;
var $handlerNull = [{id:-2,cont:id,x:null,k:id}]
var $handlers    = [$handlerNull];

var $hstack = [];
var $hcurrent = -1;


function $copyStack( hstack, top, bottom ) {
  var snew = hstack.slice(top,bottom+1);
  for(var i = 0; i < snew.length; i++) {
    var old = hstack[i+top];
    snew[i] = { optag: old.optag, ops: old.ops, ret: old.ret, readonly: false,
                 k: old.k, loc1: old.loc1 };
  }
  return snew;
}

function $pushStack(hstack,top,bottom) {
  if ($hstack===hstack && top-1 === $hcurrent) {
    // do we need to copy the locals?;
    var anyreadonly = false;
    for(var i = top; i <= bottom; i++) {
      if (hstack[i].readonly) {
        anyreadonly = true;
        break;
      }
    }
    if (!anyreadonly) {
      $hcurrent = bottom;  // push ourselves and any skipped handlers
      return top;
    }
    else {
      console.log("found readonly state");
    }
  }

  // create a new handler stack
  console.log("copy stack");
  $hstack = $hstack.slice(0,$hcurrent+1);    // shallow copy top part
  var top1 = $hstack.length;
  var topush = $copyStack(hstack,top,bottom); // deep copy the handlers we need to push
  $hstack = $hstack.concat(topush);          // and push them
  $hcurrent = $hstack.length-1;
  return top1;
}

function $resume1(hstack,top,bottom) {
  return function(cont,x,loc1,k) {
    var top1 = $pushStack(hstack,top,bottom);
    $hstack[top1].loc1 = loc1;
    $hstack[top1].k = k;
    return cont(x);
  }
}
function $resume0(hstack,top,bottom) {
  return function(cont,x,k) {
    var top1 = $pushStack(hstack,top,bottom);
    $hstack[top1].k = k;
    return cont(x);
  }
...
function $genericOptHandler( yld ) 
{
  do {
    if (yld._tag===1) {
      ...
      var current = $hstack[top];
      $hcurrent = top-1;
      // invoke
      if (current.loc1 === undefined) {
        yld = current.ops(yld.op._field1, 
                                      yld.cont, 
                                      $resume0($hstack,top,bottom), 
                                      current.k );
      }
      else {
        yld = current.ops(yld.op._field1, 
                                      yld.cont, 
                                      $resume1($hstack,top,bottom), 
                                      current.loc1, current.k );        
      }
    }
    else { // result
      // pop the handler
      var current = $hstack[$hcurrent];
      $hcurrent--;
      if (current.loc1 === undefined) {
        yld = current.ret(yld.result,current.k);
      }
      else {
        yld = current.ret(yld.result,current.loc1,current.k);
      }
    }
  }
  while ($hcurrent >= 0);
...
function $makeGenOptHandler1(optag,ret,ops) {
  return function(action,loc1,k) {
    if (k===undefined) k = id;
    var handler = { optag: optag, ret: ret, ops: ops, readonly: false, k : k, loc1: loc1 };    
...
function $makeGenOptHandler0(optag,ret,ops) {
  return function(action,k) {
    if (k===undefined) k = id;
    var handler = { optag: optag, ret: ret, ops: ops, readonly: false, k : k };    
    return $withGenHandler(handler,action);
  }
}

function $withGenHandler( handler, action ) {
  $hstack = $hstack.slice(0,$hcurrent+1); // copy since some resume may hold it 
  $hstack.push(handler);
  $hcurrent++;    
  if ($hcurrent === 0) {
...
function $genericHandler( yld ) 
{
  ...
  do {
    if (yld._tag===1) {
      ...
      $hstack.shift();
      // invoke
      yld = current.ops(yld.op._field1, $cont(yld.cont,popped), $resume(current), current.k );
...
function $makeOptHandler0(optag,ret,ops) {
  var self = { id: -1, cont: undefined, x : undefined, k : undefined };

  function resume(_cont,_x,_k) { 
    if ($handlers[0].id === self.id) {
      self.cont = _cont; self.x = _x; self.k = _k; return; // return undefined and unwind the stack.
    }
    else return handler(_cont,_x,_k);
  }
  function handler(_cont,_x,_k) { 
    self.cont = _cont; self.x = _x; self.k = _k; self.id = $handlersId++;
    $handlers.unshift(self);
    var res;
    do {
      var yld = self.cont(self.x);
      if (yld===undefined) {
        throw "someone tried to resume outside our frame";
      }
      else if (yld._tag === 1) {
        if (yld.op._tag === optag) {
          res = ops(yld.op._field1, yld.cont, resume, self.k);                    
        }
        else {
          res = $reyield0(yld, handler, self.k);
        }
      }
      else {
        res = ret(yld.result,self.k);       
      }   
    }
    while (res===undefined);
    $handlers.shift();
...
function $makeHandler0(optag,ret,ops) {
  ...
  var x;
  var k;
  function resume(_cont,_x,_k) { cont = _cont; x = _x; k = _k; return Resume; }
  function hndler(_cont,_x,_k) { cont = _cont; x = _x; k = _k; 
    var yld = cont(x);
    while(true) {
      if (yld._tag === 1) {
        if (yld.op._tag === optag) {
          yld = ops(yld.op._field1, yld.cont, resume, k);  
          if (yld._tag !== 3) { // Op or Ret for parents (called inside the ops)
            return $reyield0(yld,hndler,k);
          }
        }
        else { // Op for parents (unhandled by this hndler)
          return $reyield0(yld, hndler, k);
        }
      }
      else if (yld._tag === 2) { // Result
        return ret(yld.result,k);
      }
      else if (yld._tag === 3) {  // Resume 
        yld = cont(x);
      }
      else {
        throw "impossible!";
