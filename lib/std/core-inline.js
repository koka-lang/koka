...
function _double_show_exp(d,fractionDigits) {
  if (fractionDigits < 0) {
    ...
    return d.toExponential(fractionDigits);
  }
}
...
function _double_show_fixed(d, fractionDigits) {
  if (fractionDigits < 0) {
    // use at most |fractionDigits|
    var s = d.toFixed(-fractionDigits);
    var cap = /^(\d+\.)(\d+)$/.exec(s);
    if (!cap) return s;
    var frac = cap[2].substr(0,-fractionDigits);
    return cap[1] + frac.replace(/([1-9])0+$/,"$1");
  }
  else {
    if (fractionDigits > 17) fractionDigits = 17;
...
function _int_double(x) {
  return (_is_small(x) ? Math.round(x) : (isFinite(x) ? bigInt(x) : 0));
}
