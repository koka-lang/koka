...
function $genericHandler( cont, x ) 
{
  function $resume(current) {
    return function(_cont,_x,_k) {
  ...
  do {
    ...
    if (yld._tag===1) {
      ...
      $hstack.shift();
      // invoke
      res = current.ops(yld.op._field1, $cont(yld.cont,popped), $resume(current), current.k );
      if ($hstack.length===0) return res;
    }
    else { // result
      var current = $hstack[0];
      $hstack.shift();
      res = current.ret(yld.result,current.k);
      if ($hstack.length===0) return res;      
    }
  }
  while (true);
