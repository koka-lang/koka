...
{
  function $resume(hstack,top,bottom) {
    return function(cont,x,_k) {
      if ($hstack===hstack && top+1 === $hcurrent) {
        ...
        $hstack = $hstack.slice($hcurrent);     // remove bottom
        hstack[top].k = _k; // set our new k
        var topush = hstack.slice(bottom,top+1);   // take the handlers we need to push
        $hstack = topush.concat($hstack);     // and push them
        $hcurrent = 0;
      }
      return cont(x);
    };
  ...
  do {
    if (yld._tag===1) {
      ...
      var bottom = $hcurrent;
      var top    = bottom;
      while ($hstack[top].optag !== yld.op._tag) top++;
      ...
      yld = current.ops(yld.op._field1, 
                                    yld.cont, 
                                    $resume($hstack,top,bottom), 
                                    current.k );
      if ($hcurrent >= $hstack.length) return yld;
    }
    else { // result
      // pop the handler
...
function $makeGenOptHandler0(optag,ret,ops) {
  return function(action,k) {
    ...
    // push our handler
    if ($hcurrent >= $hstack.length) {
      $hstack   = [handler];
      $hcurrent = 0;
      return $genericOptHandler(action(Result));
    }
    else {
      $hstack = $hstack.slice($hcurrent); // copy since some resume may hold it 
      $hstack.unshift(handler);
      $hcurrent = 0;
