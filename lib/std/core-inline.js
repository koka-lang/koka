...
function _char_iter( s, from, f ) {
  ...
  for(var i = from; i < s.length; i++) {
    ...
    if (c >= 0xD800 && c <= 0xDBFF && i < s.length-1) {
...
function _string_to_chars(s) {
  var xs = [];
  _char_iter(s, 0, function(c,i) { xs.push(c); });
  return xs;
}

function _string_char_count(s) {
  var count = 0;
  _char_iter(s, 0, function(c,i) { count++; } );
  return count;
}
...
function _sslice_advance( slice, count ) {
  ...
  var idx = slice.start;
  var end = slice.start + slice.len;  
  var slicecount = _sslice_count(slice);
  if (count > 0) {
