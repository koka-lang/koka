...
var $exnTag = _Eff__hexn()._tag; // get the hexn effect tag
...
function $genericOptHandler( yld ) 
{
  do {
    if (yld._tag===1) {
      ...
      // if not found: cannot happen in a typed setting, except for exception handlers
      if (top < 0) {
        if (yld.op._tag===$exnTag) throw yld.op._field1._field1; // rethrow
...
function $genericOptHandlerExn( yld ) {
  while(true) {
    try {
      return $genericOptHandler(yld);
    }
    catch(exn) {
      ...
      if (top < 0) {
        ...
        yld = hthrow(exn, function(x) { errorNoResume("exn") });
