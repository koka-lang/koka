  ...
  for(var i = (from!=null ? from : s.length-1); i >= 0; i--) {
    ...
    if (_is_low_surrogate(c) && i > 0) {
      ...
      if (_is_high_surrogate(hi)) {
        i--;
        c = ((hi - 0xD800) * 0x0400) + (c - 0xDC00) + 0x10000;
      }
    }
    if (f(c,i,i0+1)) break;
  }
...
function _sslice_first( s ) {
  var len;
  if (s.length===0) len = 0;
  else if (_is_high_surrogate(s[0])) len = 2
  else len = 1;
...
function _sslice_last( s ) {
  var len;
  if (s.length===0) len = 0;
  else if (_is_low_surrogate(s[s.length-1])) len = 2
  else len = 1;
...
function _sslice_next( slice ) {
  ...
  else if (_is_high_surrogate(slice.str[slice.start]) && slice.len > 1 && _is_low_surrogate(slice.str[slice.start+1])) {
    var c = ((slice.str[slice.start] - 0xD800) * 0x0400) + (slice.str[slice.start+1] - 0xDC00) + 0x10000;
    return Just( {fst: c, snd: { str: slice.str, start: slice.start+2, len: slice.len-2 }} );
  }
  else {
    return Just( {fst: slice.str[slice.start], snd: { str: slice.str, start: slice.start+1, len: slice.len-1 }});
  }
}
