...
function _gformat(x,format) {
  ...
  if (fix) {
    ...
    return (w>0 && w<=20 ? x.toFixed(w) : _trimzeros(x.toFixed(20)));
  }
...
function _int_double(x) {
  return (_is_small(x) ? Math.round(x) : bigInt(x));
}
...
function _big_div_pow10(x,n)  { return bigInt(x).div_pow10(n); }
function _big_mul_pow10(x,n)  { return bigInt(x).mul_pow10(n); }
...
function _big_divmod(x,y) {
  ...
  var r = qr.remainder;
  if (r.isNegative()) {
    if (d.isPositive()) { q = q.next(); r = r.add(d) }
                   else { q = q.prev(); r = r.subtract(d) }
  }
