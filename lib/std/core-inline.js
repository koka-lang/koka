...
function $makeHandler0(optag,ret,ops) {
  ...
  var x;
  var k;
  function resume(_cont,_x,_k) { cont = _cont; x = _x; k = _k; }
  function hndler(_cont,_x,_k) { cont = _cont; x = _x; k = _k; 
    ...
    do {
      ...
      if (yld._tag === 1) {
        ...
        else res = $reyield0(yld, hndler, k);
      }
      else res = ret(yld.result,k);
    }
    while(res===undefined);
    return res;
  }
    ...
    while(true) {
      if (yld._tag === 1) {
        if (yld.op._tag === optag) {
          ...
          if (yld._tag === 1) { // Op for parents (called inside the ops)
        ...
        //if (yld._tag === 2) return yld.result;
      }
      else if (yld._tag === 3) {  // Resume 
        yld = cont(x);
      }
      else if (yld._tag === 4) {  // Done
        return yld.done;
      }
      else {
        throw "impossible!";
