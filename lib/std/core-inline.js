...
function _gformat(x,format) {
  ...
  if (fix) {
    ...
    return (w>0 && w<=20 ? x.toFixed(w) : x.toFixed());
  }
...
function _int_sign(x) {
  if (!isNaN(x)) {
    ...
    const c = x.sign();
    return (x.isZero() ? Eq : (c.isPositive() ? Gt : Lt));
  }
}
