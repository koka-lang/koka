...
function $genericOptHandler( yld ) 
{
  function $resume1(hstack,top,bottom) {
    return function(cont,x,loc1,_k) {
      hstack[top].loc1 = loc1;
      return $resume(hstack,top,bottom,cont,x,_k);
    }
  }
  ...
  function $resume(hstack,top,bottom,cont,x,_k) {
    hstack[top].k = _k; // set our new k
    if ($hstack===hstack && top-1 === $hcurrent) {
      // re-use the stack; restore locals
      for(var i = top+1; i <= bottom; i++) {
        hstack[i].loc1 = hstack[i].loc1x;
      }
      ...
      throw "no";
      $hstack = $hstack.slice(0,$hcurrent+1);    // copy top part
      var topush = hstack.slice(top,bottom+1);   // take the handlers we need to push
  ...
  do {
    if (yld._tag===1) {
      ...
      while ($hstack[top].optag !== yld.op._tag) {
        $hstack[top].loc1x = $hstack[top].loc1;
        top--;
      }
...
function $makeGenOptHandler1(optag,ret,ops) {
  return function(action,loc1,k) {
    if (k===undefined) k = id;
    var handler = { optag: optag, ret: ret, ops: ops, k : k, loc1: loc1 };    
...
function $makeGenOptHandler0(optag,ret,ops) {
  return function(action,k) {
    if (k===undefined) k = id;
    var handler = { optag: optag, ret: ret, ops: ops, k : k };    
    return $withGenHandler(handler,action);
  }
}
