...
var handlerFrameCount = 0;
var currentFrame = 0;

function $makeOptHandler0(optag,ret,ops) {
  var myFrame = handlerFrameCount++;
  var cont;
  var x;
  var k;
  function resume(_cont,_x,_k) { 
    if (myFrame === currentFrame) {
      cont = _cont; x = _x; k = _k; return; // return undefined and unwind the stack.
    }
  ...
  function handler(_cont,_x,_k) { 
    ...
    cont = _cont; x = _x; k = _k; 
    while(true) {
      var res;
      var yld = cont(x);
      if (yld===undefined) {
        throw "some tried to resume outside our frame";
      }
      else if (yld._tag === 1) {
        if (yld.op._tag === optag) {
          var res = ops(yld.op._field1, yld.cont, resume, k);          
          ...
          currentFrame = previousFrame;
          return $reyield0(yld, handler, k);
        }
      }
      else {
        res = ret(yld.result,k);
        if (res===undefined) continue;
        currentFrame = previousFrame;
        return res;
      }   
