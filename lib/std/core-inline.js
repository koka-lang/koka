...
// TODO put the range info on the Range object ?
function exn_error_min(number, min) {
  throw new _Exception( "number cannot be smaller than " + min + " but it was " + number, Range );
}

// TODO put the range info on the Range object ?
function exn_error_max(number, max) {
  throw new _Exception( "number cannot be bigger than " + max + " but it was " + number, Range );
}
...
function exn_message( exn ) {
  ...
  else if (typeof exn.message === "string") {
    msg = exn.message;
  }
...
function _check_bounds(start, index, end) {
  // This is triggered when the array's length is 0
  if (start > end) {
    // TODO better error for this
    exn_error_range();

  } else if (index < start) {
    exn_error_min(index, start);

  } else if (index > end) {
    exn_error_max(index, end);
  }
}

function _vector_copy(a) {
  var length = a.length;
  var out = new Array(length);

  // TODO benchmark whether this is the fastest way to copy an Array
  while (length--) {
    out[length] = a[length];
  }
...
function _vector_insert(a, v) {
  var length = a.length;
  var out = new Array(length + 1);
...
function _unsafe_vector_insert_at(a, index, v) {
  var length = a.length;
...
function _vector_insert_at(a, index, v) {
  index = _int_to_int32(index); // TODO is this correct ?
  _check_bounds(0, index, a.length);
  return _unsafe_vector_insert_at(a, index, v);
}

function _unsafe_vector_replace_at(a, index, v) {
  var out = _vector_copy(a);
  out[index] = v;
  return out;
}

function _vector_replace_at(a, index, v) {
  index = _int_to_int32(index); // TODO is this correct ?

  _check_bounds(0, index, a.length - 1);
  ...
  if (a[index] === v) {
    ...
    return _unsafe_vector_replace_at(a, index, v);
  }
}

function _unsafe_vector_remove_at(a, index) {
...
function _vector_remove_at(a, index) {
  index = _int_to_int32(index); // TODO is this correct ?
  _check_bounds(0, index, a.length - 1);
  return _unsafe_vector_remove_at(a, index);
}

function _unsafe_vector_push_all(out, a) {
  var length = a.length;

  if (length > 0) {
    var length2 = out.length;

    // TODO is this faster than using push ?
    out.length = length2 + length;
...
function _vector_flatten(a) {
  ...
  // Optimization
  case 2:
    return _vector_concat(a[0], a[1]);

  // TODO if there is exactly 1 non-empty sub-vector, just return that rather than a new vector
  // TODO if all of the sub-vectors are empty, return the first sub-vector rather than a new vector
  default:
    // Unfortunately we don't know what the length of the array should be, so we can't pre-initialize it
    var out = [];

    for (var i = 0; i < length1; ++i) {
      _unsafe_vector_push_all(out, a[i]);
    }
...
function _vector_concat(a, b) {
  ...
  var out = new Array(length_a + length_b);
...
// TODO verify that this is statistically correct
// https://en.wikipedia.org/wiki/Fisher%E2%80%93Yates_shuffle
function _vector_shuffle(a) {
  var length = a.length;
...
function _unsafe_vector_slice(a, start, end) {
  var diff = end - start;
  var out = new Array(diff);
...
function _vector_slice(a, start, end) {
  start = _int_to_int32(start); // TODO is this correct ?
  end = _int_to_int32(end); // TODO is this correct ?
  ...
  if (start === 0 && end === length) {
    ...
    if (start > end) {
      exn_error_max(start, end);
    }

    // TODO what about integer overflow for the length ?
    _check_bounds(0, start, length);
    _check_bounds(0, end, length);

    return _unsafe_vector_slice(a, start, end);
  }
}

// TODO cast size to int32 ?
function _vector_chunks(a, size) {
  if (size < 1) {
    exn_error_min(size, 1);
  }
  ...
  // Optimization
  // TODO error if the size is greater than the length ?
  } else if (size >= length) {
    return [a];

  } else {
    var out = new Array(Math.ceil(length / size));

    var index = 0;

    // TODO can this be made faster ?
    for (var i = 0; i < length; i += size) {
      out[index] = _unsafe_vector_slice(a, i, Math.min(i + size, length));
      ++index;
    }
...
function _vector_zip(a) {
  ...
  if (length === 0) {
    ...
    for (;;) {
      // TODO on the final iteration, this is unused, figure out a way to fix that
      var out2 = new Array(length);
...
// TODO make this faster
// TODO test this
function _vector_zip_default(a, def) {
  var length = a.length;
...
function _vector_zip2(a, b, fn) {
  var length = Math.min(a.length, b.length);

  // TODO pre-initialize the size of the array ?
  var out = new Array(length);
...
// TODO make this faster
function _vector_zip2_default(a, b, def1, def2, fn) {
  var length_a = a.length;
  var length_b = b.length;
...
function _vector_range(start, end) {
  if (start === end) {
    return [];
...
function _int_double(x) {
  if (_is_small(x)) return Math.round(x);
...
function _double_parse(s) {
  if (s==="") return Nothing;
  const d = parseFloat(s);
  return (isNaN(d) ? Nothing : Just(d));
}
