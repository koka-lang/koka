...
function $xdirectMakeHandler1(optag,matchop,res,ops) {
  function hndler(yld,loc,k) {
    var tailcall;
          
    function resume(istailcall,x,loc1,k1) { 
      if (istailcall) {
        loc = loc1;
        k   = k1;
        tailcall = true;
        ...
        return hndler(yld.resume(x),loc1,k1);
      } 
    };
    while(true) {      
      if (yld._tag === 1) {
        var op = yld.operation; // matchop(yld.operation);        
        if (op._tag === optag) {
          tailcall = false;
          var x = ops(op._field1, resume,  loc, k);
          if (tailcall) {
            yld = yld.resume(x);
            continue;
          }
          ...
          return reyield(yld.operation, function(x) { return hndler(yld.resume(x),loc,k); } );
        }
      }
      else {
        return res(yld.result, loc, k);
      }
    }    
  }

  return unsafeMakeHandler1(hndler);
}

function $xdirectMakeHandler0(optag,matchop,res,ops) {
  function hndler(yld,k) {
    var tailcall;
          
    function resume(istailcall,x,k1) { 
      if (istailcall) {
        k   = k1;
        tailcall = true;
        return x;
    ...
    while(true) {      
      if (yld._tag === 1) {
        ...
        if (op._tag === optag) {
          ...
          return reyield(yld.operation, function(x) { return hndler(yld.resume(x),k); } );
        }
      }
      else {
        return res(yld.result,k);
      }
    }    
  }

  return unsafeMakeHandler0(hndler);
}
function $directMakeHandler1(optag,matchop,res,ops) {
  var action;
  var x;
  var loc;
  var k;
  function hndler1(action0,x0,loc0,k0) { 
    action = action0;
    x = x0;
    loc = loc0;
    k = k0;
    while(true) 
    {
      ...
      if (yld._tag === 1) {
        var op = yld.operation; // matchop(yld.operation);        
        if (op._tag === optag) {
          var r = ops(op._field1, (function(yresume) {
                                     return function(istailcall,x1,loc1,k1) {
                                              x = x1; loc = loc1; k = k1; action = yresume;
                                              return;
                                            };
                                  })(yld.resume), loc, k);
          ...
          return Yield(yld.operation, (function(yresume,yloc) {
                                          return function(y) { return hndler1(yresume, y, yloc, k); };
                                      })(yld.resume,loc) );
          //return reyield(yld.operation, function(x) { return hndler(yld.resume(x),loc,k); } );
        }
      }
      else {
        var r = res(yld.result, loc, k);
        if (r !== undefined) return r;
      }
    }    
  }
  return function(action,loc,k) {
    if (k===undefined) k = id;
    return hndler1(action,Result,loc,k);
  };
}
function $directMakeHandler0(optag,matchop,res,ops) {
  var action;
  var x;
  var k;
  function hndler0(action0,x0,k0) { 
    action = action0
    x = x0;
    k = k0;
    while(true) {
      ...
      if (yld._tag === 1) {
        ...
        if (op._tag === optag) {
          var r = ops(op._field1, (function(yresume) { 
                                    return function(istailcall,x1,k1) {
                                      x = x1; k = k1; action = yresume;
                                      return;
                                    }
                                  })(yld.resume), k);
          if (r !== undefined) {
          ...
          return Yield(yld.operation, (function(yresume) {
                                          return function(y) { return hndler0(yresume, y, k); };
                                      })(yld.resume) );
          // return reyield(yld.operation, function(x) { return hndler(yld.resume,x,k); } );          
        }
      }
      else {
        var r = res(yld.result,k);
        if (r !== undefined) {
          return r;
        }
      }
  ...
  return function(action,k) {
    if (k === undefined) k = id;
    return hndler0(action,Result,k);
  };
}