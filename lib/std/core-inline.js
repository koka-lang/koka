...
function $copyHandler( h ) {
  return { optag: h.optag, ops: h.ops, ret: h.ret, readonly: false,
           k: h.k, loc1: h.loc1 };
}
...
function $pushStack(hstack,top,bottom) {
  ...
  if ($hstack[top1].readonly) {
    console.log(" found readonly; copy handler: " + $hstack[top1].optag);
    if ($hstack===hstack) {
      console.log(" shallow copy");
      $hstack = $hstack.slice(0); // shallow copy so it gets a different 'id' and we don't
                                // overwrite the 'readonly' handler
    }
    $hstack[top1] = $copyHandler($hstack[top1]); // deep copy this handler
  }
  return $hstack[top1];  
}
...
function $resume1(hstack,top,bottom) {
  return function(cont,x,loc1,k) {
    var h  = $pushStack(hstack,top,bottom);
    h.loc1 = loc1;
    h.k    = k;
    return cont(x);
  }
