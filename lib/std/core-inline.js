...
function $resume1(hstack,topbot,cont,loc1,x,k) {
...
function $makeGenOptHandler1(optag,ret,ops) {
  return function(loc1,action,k) {
    ...
    return $withGenHandler( handler, action, k===undefined );
  };
}
...
function $withGenHandler( handler, action, startCps ) {
  var hstack;
  var htop;
  try {
    if (startCps) {
...
function $reyield1( yld, hndler, loc, k ) {
  var cont = yld.cont;
  yld.cont = function(x) { return hndler(cont,loc,x,k); };
  return yld;
}
