...
typedef kk_datatype_t kk_evv_t;         // either a kk_evv_vector_t, or a single evidence, or empty
                                        // todo: make testing this faster by representing empty vector as an allocated vector? 

static inline kk_evv_t kk_evv_empty(kk_context_t* ctx) {
  return kk_datatype_from_tag(2);
}

static inline bool kk_evv_is_empty(kk_evv_t evv) {
  return kk_datatype_is_singleton(evv);
}

static inline bool kk_evv_is_vector(kk_evv_t evv) {
  return kk_datatype_has_tag(evv,KK_TAG_EVV_VECTOR);
}
...
static inline struct kk_std_core_hnd__ev_s* kk_evv_at( size_t i, kk_context_t* ctx ) {
  ...
  kk_assert_internal(!kk_datatype_is_singleton(evv)); // empty
  if (!kk_datatype_has_tag(evv,KK_TAG_EVV_VECTOR)) {  // evv is a single evidence
    kk_assert_internal(i==0);
    return kk_datatype_as(struct kk_std_core_hnd__ev_s*,kk_evv_dup(evv));
  }
  else {  // evv as a vector
    kk_evv_vector_t vec = kk_datatype_as_assert(kk_evv_vector_t, evv, KK_TAG_EVV_VECTOR);
    kk_assert_internal(i < (kk_block_scan_fsize(&vec->_block) - 1));
...
static inline kk_evv_t kk_evv_swap_create1(size_t i, kk_context_t* ctx) {
  ...
  if (kk_evv_is_vector(evv0)) {
    ctx->evv = kk_datatype_from_ptr((kk_block_t*)kk_evv_at(i, ctx)); // cast as ev struct is not defined yet 
    return evv0;
  }
  else {      
    kk_assert_internal(i==0);
    kk_assert_internal(!kk_evv_is_empty(evv0));
