...
function _evv_delete(w,i,behind) {
  if (behind) i++;
  const ofs = w.ofs;
  const evv = w.evv;
  const n = evv.length - ofs;
...
function _evv_get() {
  return { evv: $std_core_hnd._evv, ofs: $std_core_hnd._evv_ofs };
}

function _evv_set(w) {
  $std_core_hnd._evv = w.evv;
  $std_core_hnd._evv_ofs = w.ofs;
}
...
function _evv_swap_create1(i) {
  const w = _evv_get();
  $std_core_hnd._evv_ofs = i;
  // $std_core_hnd._evv_ofs = 0;
  // $std_core_hnd._evv = [$std_core_hnd.evv[i]];
  return w;
}
...
function _evv_swap_create(indices) {
  const w = _evv_get();
  _evv_set(w,_evv_create(indices));
  return w;
}
