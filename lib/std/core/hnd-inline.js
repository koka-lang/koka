...
$std_core_hnd = $std_core._export( $std_core_hnd, {
                  "_evv_ofs": 0,
                  "_evv"    : [],
                  "_yield"  : null,
                });
...
function _evv_index(evv,ofs,tag) {
  for(var i = ofs; i < evv.length; i++) {
...
function _guard(w) {
  if (!($std_core_hnd._evv === w.evv && $std_core_hnd._evv_ofs === w.ofs)) {
    $std_core.error("trying to resume outside the (handler) scope of the original handler. \n captured under:\n" + _evv_show(w) + "\n but resumed under:\n" + _evv_show({evv:$std_core_hnd._evv, ofs: $std_core_hnd._evv_ofs }));
  }
}
...
function _yield_prompt(m) {
  if ($std_core_hnd._yield === null) {
    ...
    const cont   = ($std_core_hnd._yield.final ? $std_core.Nothing : $std_core.Just(_kcompose(0,$std_core_hnd._yield.conts_count,$std_core_hnd._yield.conts)));
