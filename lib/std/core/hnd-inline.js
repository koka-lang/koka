...
function _evv_insert(evv,ev) {
  // update ev
  if (ev._field2===0) return; // marker zero is ev-none 
  ev._field5 = evv;
  if (ev._field2 < 0) {
    evv._cfc = _cfc_lub(evv._cfc, ev._field4);  // control flow context    
    return; // a negative (named) marker is not in the evidence vector
  }
  // insert in the vector
  const n    = evv.length;
  const evv2 = new Array(n+1);
  evv2._cfc = _cfc_lub(evv._cfc, ev._field4); 
  var i;
  for(i = 0; i < n; i++) {
    const ev2 = evv[i];
...
function _evv_delete(evv,i,behind) {
  ...
  // update cfc?
  if (evv[i]._field4 >= evv._cfc) {
    var cfc = evv2[0]._field4; 
    for(j = 1; j < n-1; j++) {
      cfc = _cfc_lub(evv2[j]._field4, cfc);
    }
    evv2._cfc = cfc;
  } 
  return evv2;
...
function __evv_index(evv,tag) {
  const n = evv.length
  for(var i = 0; i < n; i++) {
    if (tag <= evv[i]._field1) return i;  // string compare
  }
  return n;
}
...
function _guard(evv) {
  if ($std_core_hnd._evv !== evv) {
    if ($std_core_hnd._evv.length == evv.length) {
      var equal = true;
      for(var i = 0; i < evv.length; i++) {
        if ($std_core_hnd._evv._field2 != evv._field2) {
          equal = false;
          break;
        }
      }
