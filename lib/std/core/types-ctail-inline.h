...
static inline kk_box_t kk_ctail_resolve( kk_std_core_types__ctail acc, kk_box_t child, kk_context_t* ctx ) {
  #if !defined(KK_CTAIL_NO_CONTEXT_PATH)
  // note: written like this for best codegen; be careful when rewriting.
  if (kk_likely(acc.hole != NULL && kk_block_is_unique(kk_ptr_unbox(acc.res)))) {
...
static inline kk_std_core_types__ctail kk_ctail_link( kk_std_core_types__ctail acc, kk_box_t child, kk_box_t* field, kk_context_t* ctx  ) {
  return kk_std_core_types__new_CTail( kk_ctail_resolve(acc,child,ctx), field, ctx );
}
