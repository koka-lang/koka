...
function _Callback( cb, haserr, nodup ) {
  ...
  self.hctx = $std_core._save_handler_context();
  // remember if the callback itself was constructed inside a sync block
  self.in_synchronous = (_syncNesting > 0); 
...
_Callback.prototype.invoke = function(args) {
  ...
  if (!self.in_synchronous && _syncNesting > 0) { // if so, put ourselves on the queue
    ...
    _syncBlocked.push( { cback: self, args: args } );
    return;
  }
  else {
