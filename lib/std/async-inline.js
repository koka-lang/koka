...
function _create_cancelable( caction ) {
  ...
  function cancel()   { return _cancel(loc); }
  function action(_k) { return caction( cancel, _k); }
  function cb() { return loc; }; // just ensure it is a newly created thunk
  cb.hctx = null;
  cb.cancel = null;
  return $std_core._tuple2_( 
    (function(){ _unregister_cancel(cb); }),
    (function(_k){ 
      _register_cancel(cb,cancel);
      return _cancel_handler( loc, action, _k ); 
    })
  );
}
