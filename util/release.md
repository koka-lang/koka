...
```sh
curl -sSL https://github.com/koka-lang/koka/releases/download/v3.3.0/install.sh | sh
```
### Windows (x64, arm64)

Open a `cmd` prompt and download and run the installer:
```sh
curl -sSL -o %tmp%\install-koka.bat https://github.com/koka-lang/koka/releases/download/v3.3.0/install.bat && %tmp%\install-koka.bat
```
