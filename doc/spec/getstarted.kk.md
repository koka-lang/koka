...
The excellent performance relative to C++ here (on Ubuntu with an AMD 3600XT) 
...
For example, we can load the ``rbtree`` example again and print out the elapsed runtime with ``--showtime``:
    ...
    ...
    420000
    info: elapsed: 1.119s, user: 1.093s, sys: 0.031s, rss: 230mb
...
and then enable optimizations with ``-O2`` and run again (on Windows with an AMD 5950X):
    ...
    ...
    420000
    info: elapsed: 0.822s, user: 0.828s, sys: 0.000s, rss: 198mb
